...
(after! format
  ...
  ;;   (shell-command-on-region (point-min) (point-max) "shfmt" nil "*shfmt-error*" t)
...
(defun sa/write-headings-for-all-dirs (headings-file selector)
  ...
  (sa/write-headings-for-dir "~/.doom.d" headings-file selector)
  (sa/write-headings-for-dir "~/notes" headings-file selector)
  )
(defun sa/write-headings-for-agenda-dirs (headings-file selector)
  (delete-file headings-file)
  (sa/write-headings-for-dir "~/notes" headings-file selector)
  )
...
(defun sa/all-headings-selector (org-file)
  (org-ql-query
    ;; Use (sa/get-org-heading-and-content-single-line) to get content as well.
...
(defun sa/due-headings-selector (org-file)
  (org-ql-query
    ...
    :select '(org-get-heading)
    :from org-file
    :where '(or (and (or (deadline :to +1)
                     (scheduled :to +1)
                     (ts-active :from today :to +1))
                 (not (done))
                 )
                (and (level 1) (tags "mail"))
                )
    :order-by '(date priority todo)
    ))
  (defun sa/write-due-headings (headings-file)
    (sa/write-headings-for-agenda-dirs headings-file #'sa/due-headings-selector ))
#+END_SRC
...
#+BEGIN_SRC elisp
  ...
  (setq doom-variable-pitch-font (font-spec :family "Literata" :size 18))
...
(defun sa/setup-org-mode (orgdir)
  ...
  (setq org-agenda-files '("~/notes" "~/work-notes"))
  ...
  (setq org-capture-templates
        '(
          ("t" "Todo" entry (file "~/notes/inbox.org")
           "* TRIAGE %? %^G\n%i\n%x\n")
          ("e" "Todo from email" entry (file "~/notes/inbox.org")
           "* TRIAGE %:subject %^G\n%?\n %i\n %a\n")
          ("l" "Todo from link" entry (file "~/notes/inbox.org")
          ...
          ("B" "Manual Bookmark" entry (file+headline "~/notes/bookmarks.org" "Bookmarks")
           "* %? %^G\nBookmarked on %U"
           )
